...
playGame game role (x,y) | terminal game = (drawGrid game) >> endGame game
                           | role == humanPlayer = (drawGrid game) >> getMove game >>= (\mv -> playGame (playMove game humanPlayer mv) (switch humanPlayer) (x,y))
...
-- compMove generate the move of computer player, which shold get the step that make the chance of human wining minimum
compMove::(Role->Game->Int)->Game->Game
compMove fn game | length (elemIndices emptyCell game) < searchSpace = snd $ head $ sortBy (compare `on` fst) $ map (\i-> (fn humanPlayer i, i)) (moves game compPlayer)  -- looking for the minimum value for computer player
